"""
Tensor Based Context-Free Path Querying
"""
...
def rsm_to_nfa(automaton: RecursiveAutomaton) -> NondeterministicFiniteAutomaton:
    ...
    for rule, box in automaton.boxes.items():
        dfa = box.dfa

        start_final_states = dfa.start_states.union(dfa.final_states)
        for state in start_final_states:
...
def tensor_based_cfpq(
    ...
    for non_terminal in rsm.boxes:
        # if graph_adj_matrix.adj_matrices.get(non_terminal) is None:
        if non_terminal not in graph_adj_matrix.adj_matrices:
            states_count_graph = graph_adj_matrix.states_count
            dim = (states_count_graph, states_count_graph)
            graph_adj_matrix.adj_matrices[non_terminal] = sp.sparse.csc_matrix(
                dim, dtype=bool
            )
        # if rsm_adj_matrix.adj_matrices.get(non_terminal) is None:
        if non_terminal not in rsm_adj_matrix.adj_matrices:
            states_count_rsm = rsm_adj_matrix.states_count
            dim = (states_count_rsm, states_count_rsm)
            rsm_adj_matrix.adj_matrices[non_terminal] = sp.sparse.csc_matrix(
                dim, dtype=bool
            )
    ...
    while prev_nonzero_count != new_nonzero_count:
        ...
        for idx in range(len(src_indices)):
            ...
            if src_rsm_node in src_rsm_states and dest_rsm_node in dest_rsm_states:
                graph_adj_matrix.adj_matrices[src_symbol][
                    graph_adj_matrix.states_indices[src_graph_node],
                    graph_adj_matrix.states_indices[dest_graph_node],
                ] = True

        new_nonzero_count = 0
        for _, adj_matrix in graph_adj_matrix.adj_matrices.items():
    ...
    for start_state in graph_adj_matrix.start_states:
        for final_state in graph_adj_matrix.final_states:
            if graph_adj_matrix.adj_matrices[rsm.initial_label][
